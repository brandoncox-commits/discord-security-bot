...
hardening. Built on discord.py 2.x.

All commands are invoked by **@mentioning the bot** (e.g. `@Bams Modmin Tools
help`). Mention-only invocation means the bot never clashes with other bots'
commands, and — because Discord always delivers message content for messages
that mention the bot — it works WITHOUT the privileged Message Content intent.

Commands (all require the Administrator permission)
---------------------------------------------------
@bot help                     Show this command list.
@bot bulk-purge-user <user>   Ban a user, delete their last-14-day messages, and
                              remove webhooks they created (plus those messages).
@bot audit-permissions        Audit roles, @everyone, and integrations for risk.
@bot purge-webhooks           Delete every webhook in the server.
@bot panic <lock|unlock>      Freeze / restore all text channels during a raid.
@bot wipe-invites             Delete all active invite links.
@bot trace-app <app>          Find the user(s) behind an app/bot.
...
class ModminBot(commands.Bot):
    def __init__(self) -> None:
        ...
        # NOTE: Message Content intent is intentionally NOT enabled. Because every
        # command requires an @mention, Discord delivers the content we need.
        super().__init__(
            command_prefix=commands.when_mentioned,  # mention-only: no text prefix
            intents=intents,
            help_command=None,  # we ship a custom help command below
    ...
    async def on_ready(self) -> None:
        ...
        await self.change_presence(
            activity=discord.Activity(
                type=discord.ActivityType.listening, name="@mention help"
            )
        )
...
async def help_command(ctx: commands.Context) -> None:
    ...
    embed = discord.Embed(
        ...
        description=(
            f"All commands are used by mentioning me first, e.g.\n"
            f"{mention} `bulk-purge-user @baduser`\n\n"
            f"Every command requires the **Administrator** permission."
        ),
        color=discord.Color.blurple(),
        timestamp=utcnow(),
    )
    for name, usage, desc in COMMAND_HELP:
        field_name = f"{name} {usage}".strip()
...
@bot.command(name="bulk-purge-user")
@commands.guild_only()
@commands.has_permissions(administrator=True)
@commands.bot_has_permissions(ban_members=True, manage_messages=True)
async def bulk_purge_user(ctx: commands.Context, user: discord.User) -> None:
    guild = ctx.guild
    ...
    async with ctx.typing():
        ...
        try:
            await guild.ban(
                user,
                reason=f"bulk-purge-user by {ctx.author} ({ctx.author.id})",
            ...
            await ctx.send(f"❌ Failed to ban user: {exc}")
            return

        # 2. Find and remove webhooks created by the target. Malicious apps spam
        #    via webhooks, whose messages have a webhook author (not a user), so
        #    deleting the webhook also lets us catch its messages by webhook_id.
        #    Scoped to webhooks this user created so legit integrations are safe.
...
@bot.command(name="audit-permissions")
@commands.guild_only()
@commands.has_permissions(administrator=True)
async def audit_permissions(ctx: commands.Context) -> None:
    guild = ctx.guild
...
@bot.command(name="purge-webhooks")
@commands.guild_only()
@commands.has_permissions(administrator=True)
@commands.bot_has_permissions(manage_webhooks=True)
async def purge_webhooks(ctx: commands.Context) -> None:
    guild = ctx.guild
    ...
    async with ctx.typing():
        ...
        try:
            ...
            await ctx.send("❌ I need the `Manage Webhooks` permission to do this.")
            return

        for wh in webhooks:
            channel_name = getattr(wh.channel, "name", "unknown")
            try:
                await wh.delete(reason=f"purge-webhooks by {ctx.author}")
...
async def panic(ctx: commands.Context, action: str) -> None:
    action = action.lower()
    if action not in ("lock", "unlock"):
        await ctx.send("Usage: `@mention panic <lock|unlock>`")
        return
    ...
    async with ctx.typing():
        if action == "lock":
            await _panic_lock(ctx, guild, everyone, state, gid)
        else:
            await _panic_unlock(ctx, guild, everyone, state, gid)


async def _panic_lock(ctx, guild, everyone, state, gid) -> None:
    if gid in state:
        await ctx.send(
    ...
    for channel in guild.text_channels:
        ...
        try:
            await channel.set_permissions(
                everyone, overwrite=overwrite, reason=f"panic lock by {ctx.author}"
            )
...
async def _panic_unlock(ctx, guild, everyone, state, gid) -> None:
    saved = state.get(gid)
    if not saved:
        await ctx.send(
    ...
    for channel_id, prior in saved.get("channels", {}).items():
        ...
        try:
            await channel.set_permissions(
                everyone, overwrite=overwrite, reason=f"panic unlock by {ctx.author}"
            )
            restored += 1
        except discord.Forbidden:
            failed += 1
...
@bot.command(name="wipe-invites")
@commands.guild_only()
@commands.has_permissions(administrator=True)
@commands.bot_has_permissions(manage_guild=True)
async def wipe_invites(ctx: commands.Context) -> None:
    guild = ctx.guild
    ...
    async with ctx.typing():
        try:
            ...
            await ctx.send("❌ I need the `Manage Server` permission to read invites.")
            return
        ...
        for invite in invites:
            try:
                await invite.delete(reason=f"wipe-invites by {ctx.author}")
                deleted += 1
            except discord.HTTPException as exc:
    ...
    embed = discord.Embed(
        title="Invite Wipe",
        description=(
            f"All active invites have been revoked by {ctx.author.mention}. "
...
@bot.command(name="trace-app")
@commands.guild_only()
@commands.has_permissions(administrator=True)
async def trace_app(
    ctx: commands.Context, app: discord.User, scan_messages: bool = True
) -> None:
    guild = ctx.guild
    ...
    async with ctx.typing():
        ...
        def note(user: discord.abc.User | None, reason: str, msgs: int = 0) -> None:
            ...
            entry = candidates.setdefault(
                user.id, {"obj": user, "reasons": set(), "messages": 0}
            )
    ...
    embed.set_footer(text=f"Scanned {scanned_channels} channel(s) • by {ctx.author}")
    await ctx.send(embed=embed)
    await send_mod_log(guild, embed)
...
async def on_command_error(ctx: commands.Context, error: commands.CommandError) -> None:
    if isinstance(error, commands.CommandNotFound):
        return  # ignore: someone mentioned the bot without a valid command
    if isinstance(error, commands.MissingPermissions):
        ...
        message = f"⚠️ Missing argument: `{error.param.name}`. Mention me with `help` for usage."
    elif isinstance(error, (commands.UserNotFound, commands.MemberNotFound, commands.BadArgument)):
        message = f"⚠️ Couldn't parse that argument: {error}"
    else:
